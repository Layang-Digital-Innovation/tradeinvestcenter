...
class ChatService {
  async startChat(payload: { type?: 'TRADING_SUPPORT' | 'INVESTMENT_INQUIRY' | 'GENERAL_SUPPORT'; projectId?: string; title?: string }) {
    const { data } = await axiosInstance.post('/api/chat/start', payload);
    return data as { id: string };
  }

  async startChatWith(targetUserId: string, payload?: { type?: 'TRADING_SUPPORT' | 'INVESTMENT_INQUIRY' | 'GENERAL_SUPPORT'; title?: string }) {
    const { data } = await axiosInstance.post('/api/chat/start-with', { targetUserId, ...(payload || {}) });
    return data as { id: string };
  }

  async listMyChats() {
    const { data } = await axiosInstance.get('/api/chat/my');
  ...
  async getMessages(chatId: string, opts?: { cursor?: string; limit?: number }) {
    ...
    const { data } = await axiosInstance.get(`/api/chat/${chatId}/messages${qs.toString() ? `?${qs.toString()}` : ''}`);
    return data as ChatMessage[];
  }

  async postMessage(chatId: string, content: string) {
    const { data } = await axiosInstance.post(`/api/chat/${chatId}/messages`, { content });
    return data as ChatMessage;
  }
  ...
  async uploadAttachments(chatId: string, files: File[]) {
    ...
    const { data } = await axiosInstance.post(`/api/chat/${chatId}/attachments`, form, {
